...
setwd("C:/meta2016") 
j_file2 <-"C:/meta2016/talk_free.json" 
...
talk_names<-names(talk_free$talks[[1]]$talk) 
talk_names 


###Convertobject type 
talk_free_list<-lapply(talk_free$talks, function(x){unlist(x)}) 
str(talk_free_list) 
parse_talk_all<-data.frame() 
df_parse_talk_all<-do.call("rbind", c(parse_talk_all, talk_free_list)) 
...
png("./dendrogram_sparse90.png", width = 1200, height=600) 
plot(fit) 
###k= number of clusters 
rect.hclust(fit, k=12) 
dev.off() 
